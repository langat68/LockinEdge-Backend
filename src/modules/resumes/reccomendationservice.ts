...
export class RecommendationService {
  static async scrapeJobs(skills: string[], location?: string): Promise<ScrapedJob[]> {
  ...
  static async scrapeIndeedJobs(skills: string[], location?: string): Promise<ScrapedJob[]> {
  ...
  static async scrapeRemoteOkJobs(skills: string[]): Promise<ScrapedJob[]> {
    try {
      ...
      data.slice(1, 16).forEach((job: any) => {
        if (job.position && job.company) {
          const description = job.description || "";
          const matchesSkills = skills.some(
            (skill) =>
...
`;
  ...
  static async generateRecommendations(resumeId: string): Promise<JobRecommendation[]> {
    ...
    const normalizedResume = {
      ...
      analysis: resume.analysis as ResumeAnalysis,
    };
